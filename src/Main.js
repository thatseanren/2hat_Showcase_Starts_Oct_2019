...
export default class Main extends React.Component {
  constructor(props) {
    ...
    this.EditFoodList = this.EditFoodList.bind(this);
  }
  componentDidMount() {
    $($(".MuiFilledInput-inputSelect")[1]).css({
      "padding-top": "12px",
      "padding-bottom": "12px",
      height: "24px",
      width: "204px",
      ...
      timeArrow === "forwards"
        ? {
            date:
              prevState.date - 1 >= 0
                ? prevState.date - 1
                : prevState.diet.data_points.length - 1
          }
        : {
            date:
              prevState.date + 1 < prevState.diet.data_points.length
                ? prevState.date + 1
                : 0
          }
    );
  ...
  EditFoodList(food) {
    const index = this.state.diet.data_points[this.state.date].intake_list
      .length;
    this.setState(
      prevState =>
        (prevState.diet.data_points[prevState.date].intake_list[index] = food)
    );
  }
  render() {
    return (
      <App
        context={this.state}
        Datehandler={this.switchDate}
        Foodhandler={this.EditFoodList}
      />
    );
  }
}
